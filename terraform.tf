terraform {
  required_providers {
    hpcr = {
      source  = "ibm-hyper-protect/hpcr"
      version = ">= 0.1.1"
    }

    tls = {
      source  = "hashicorp/tls"
      version = ">= 4.0.1"
    }

    ibm = {
      source  = "IBM-Cloud/ibm"
      version = ">= 1.45.1"
    }
  }
}

# make sure to target the correct region and zone
provider "ibm" {
  region = var.region
  zone   = "${var.region}-${var.zone}"
}

locals {
  # some reusable tags that identify the resources created by his sample
  tags = ["hpcr", "sample", var.prefix]
}

# the VPC
resource "ibm_is_vpc" "hpvs_automation_sample_vpc" {
  name = format("%s-vpc", var.prefix)
  tags = local.tags
}

# the security group
resource "ibm_is_security_group" "hpvs_automation_sample_security_group" {
  name = format("%s-security-group", var.prefix)
  vpc  = ibm_is_vpc.hpvs_automation_sample_vpc.id
  tags = local.tags
}

# rule that allows the VSI to make outbound connections, this is required
# to connect to the logDNA instance as well as to docker to pull the image
resource "ibm_is_security_group_rule" "hpvs_automation_sample_outbound" {
  group     = ibm_is_security_group.hpvs_automation_sample_security_group.id
  direction = "outbound"
  remote    = "0.0.0.0/0"
}

# rule that allows inbound traffic
resource "ibm_is_security_group_rule" "hpvs_automation_sample_inbound" {
  group     = ibm_is_security_group.hpvs_automation_sample_security_group.id
  direction = "inbound"
  remote    = "0.0.0.0/0"
  tcp {
    port_min = 80
    port_max = 8080
  }
}

# the subnet
resource "ibm_is_subnet" "hpvs_automation_sample_subnet" {
  name                     = format("%s-subnet", var.prefix)
  vpc                      = ibm_is_vpc.hpvs_automation_sample_vpc.id
  total_ipv4_address_count = 256
  zone                     = "${var.region}-${var.zone}"
  tags                     = local.tags
}

# we use a gateway to allow the VSI to connect to the internet to logDNA
# and docker. Without a gateway the VSI would need a floating IP. Without
# either the VSI will not be able to connect to the internet despite
# an outbound rule
resource "ibm_is_public_gateway" "hpvs_automation_sample_gateway" {
  name = format("%s-gateway", var.prefix)
  vpc  = ibm_is_vpc.hpvs_automation_sample_vpc.id
  zone = "${var.region}-${var.zone}"
  tags = local.tags
}

# attach the gateway to the subnet
resource "ibm_is_subnet_public_gateway_attachment" "lhpvs_automation_sample_gateway_attachment" {
  subnet         = ibm_is_subnet.hpvs_automation_sample_subnet.id
  public_gateway = ibm_is_public_gateway.hpvs_automation_sample_gateway.id
}


# create a random key pair, because for formal reasons we need to pass an SSH key into the VSI. It will not be used, that's why
# it can be random
resource "tls_private_key" "hpvs_automation_sample_rsa_key" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

# we only need this because VPC expects this
resource "ibm_is_ssh_key" "hpvs_automation_sample_sshkey" {
  name       = format("%s-key", var.prefix)
  public_key = tls_private_key.hpvs_automation_sample_rsa_key.public_key_openssh
  tags       = local.tags
}

# locate the latest hyper protect image
data "ibm_is_images" "hyper_protect_images" {
  visibility = "public"
  status     = "available"

}

locals {
  # filter the available images down to the hyper protect one
  hyper_protect_image = [for each in data.ibm_is_images.hyper_protect_images.images : each if each.os == "hyper-protect-1-0-s390x" && each.architecture == "s390x"][0]
}

# construct the VSI
resource "ibm_is_instance" "hpvs_automation_sample_vsi" {
  name    = format("%s-vsi", var.prefix)
  image   = local.hyper_protect_image.id
  profile = var.profile
  keys    = [ibm_is_ssh_key.hpvs_automation_sample_sshkey.id]
  vpc     = ibm_is_vpc.hpvs_automation_sample_vpc.id
  tags    = local.tags
  zone    = "${var.region}-${var.zone}"

  # the user data field carries the encrypted contract, so all information visible at the hypervisor layer is encrypted
  user_data = chomp(replace(var.contract, "\\n", "\n"))

  primary_network_interface {
    name            = "eth0"
    subnet          = ibm_is_subnet.hpvs_automation_sample_subnet.id
    security_groups = [ibm_is_security_group.hpvs_automation_sample_security_group.id]
  }
}

# attach a floating IP since we would like to access the embedded server via the internet
resource "ibm_is_floating_ip" "hpvs_automation_sample_floating_ip" {
  name   = format("%s-floating-ip", var.prefix)
  target = ibm_is_instance.hpvs_automation_sample_vsi.primary_network_interface[0].id
  tags   = local.tags
}

# log the floating IP for convenience
output "ip" {
  value = resource.ibm_is_floating_ip.hpvs_automation_sample_floating_ip.address
  description = "The public IP address of the VSI" 
}


